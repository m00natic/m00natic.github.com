...
provide (subset of) SQL-like operations over them[fn:1], for example:
...
#+BEGIN_SRC C++
  bool match(const char *dbLine)
  {
      ...
      const std::regex commercialNameRegex1("PET");
      const std::regex commercialNameRegex2("MEAL");
...
The usual thing to do is embed a [[https://en.wikipedia.org/wiki/Domain-specific_language][DSL]].  For example, we could represent
...
So *select* could be a macro which transforms the above to a function
which goes map-reducing over the given DB and calls it.

#+BEGIN_SRC lisp
  (defmacro select (field-list db :where where)
    "Generate map-reduce selection code."
    ;; ...
    )
#+END_SRC
...
Where *build-field-op* based on the field operation and fields and
...
#+BEGIN_SRC lisp
  (AND (CL-PPCRE:SCAN "LH|EK" LINE :START CXR_OFFSET :END (+ CXR_OFFSET CXR_SIZE))
       (OR
        (AND (CL-PPCRE:SCAN "PET" LINE :START COMMERCIAL_NAME_OFFSET
                                       :END (+ COMMERCIAL_NAME_OFFSET COMMERCIAL_NAME_SIZE))
             (CHAR= (AREF LINE TYPE_OFFSET) #\C))
        (AND
         (CHAR= (AREF LINE TYPE_OFFSET) #\F)
         (CL-PPCRE:SCAN "MEAL" LINE :START COMMERCIAL_NAME_OFFSET
                                    :END (+ COMMERCIAL_NAME_OFFSET COMMERCIAL_NAME_SIZE))))
       (STRING< "180620" LINE :START1 0 :END1 6
                              :START2 DATE_DISC_OFFSET :END2 (+ DATE_DISC_OFFSET DATE_DISC_SIZE))
       (STRING< LINE LINE :START1 DATE_EFF_OFFSET :END1 (+ DATE_EFF_OFFSET DATE_EFF_SIZE)
                          :START2 DATE_DISC_OFFSET :END2 (+ DATE_DISC_OFFSET DATE_DISC_SIZE)))
#+END_SRC
...
bound checks if one decides so.[fn:3]
...
compilation".  Works at a higher level than [[https://en.wikipedia.org/wiki/Just-in-time_compilation][JIT]] and opens
...
language which provides *eval* function!
#+END_QUOTE
...
use [[https://en.wikipedia.org/wiki/S-expressions][S-expressions]].  What if forced to literary provide SQL syntax?
...
often take advantage of such techniques[fn:5] for difficult problems
...
[fn:3] In fact, in the same vein the *cl-ppcre* library goes a step
