...
which is for completion of anything, not only code.  Here is some
setup:
...
Emacs comes with a separate binary [[http://www.emacswiki.org/emacs/EmacsBrowse][ebrowse]][fn:built-in] that can annotate
