...
c / scheme / clojure / haskell / agda / insert your favourite niche
...
spreadsheets, [[http://emacs-w3m.namazu.org/][browsing the internetz]], [[http://emacswiki.org/emacs/ArtistMode][ASCII drawing]], [[http://www.gnu.org/software/emms/][media collection
...
[[http://emacswiki.org/emacs/DiredMode][Dired]] is the most capable file manager I've encountered.  To start it:
...
** Remote network and sudo access using TRAMP
...
The [[http://emacswiki.org/emacs/TrampMode][TRAMP]] built-in package is a killer.  It supports plethora of
...
** Don't ask for sudo or ssh passwords - authinfo
...
built-in mail/news kitchen sink [[http://www.gnus.org/][Gnus]].  Here's its glory documented (it
...
Database]] (BBDB) for managing contacts, automatic checking for new
...
and all buffers with unsaved changes are marked.  Now press:

~S~

to save them all.  Job done.  As all within La la land, IBuffer is
...
version of GNU Emacs, you can edit results in place after pressing:
...
finished.  Within it, you can just press:
...
some [[http://emacswiki.org/emacs/RegularExpression][differences]] to the popular [[http://en.wikipedia.org/wiki/Pcre][PCRE]] style and if you're serious about
...
some of it integrating with the [[http://www.gnu.org/software/gdb/][GNU Debugger]]:
...
about object under cursor, colouring of instances of local variables,
...
in terminal...yes, it does, just like rest of Emacs.  Here are some
...
Emacs comes with a separate binary [[http://www.emacswiki.org/emacs/EmacsBrowse][ebrowse]] that can annotate multiple
...
PATH (/usr/local/bin/ preferably), named ebrowse++ and made
executable:
...
It refreshes the DB for the current project and opens it in
...
This is the only environment that has enabled me learn a language and
easily add useful functionality without internet access.  And do this
with pleasure and still have hair on my head.  It's that good.
...
which normally denote package.  If it's not the case, there is even
...
/I rarely write documents and presentations.  When I do, I use Org mode./
		/-- Mahatma Gandhi/

There is another wonderful thing that comes with Emacs known as
[[http://orgmode.org/][Org mode]].  Here's what's written on their site:

"Org mode is for keeping notes, maintaining TODO lists, doing project
planning, and authoring with a fast and effective plain-text system."

This wiki entry is all done in Org Mode and then exported to HTML (one
of many available formats).  At first you'll probably be fine with the
minimum - number of stars at the beginning of line signifies heading
in the document hierarchy.  The less stars - the more important the
heading.  You can easily reorder headers at the same level (reordering
all sub-headers as well) with ~<Alt>+<Shift>+<Up>~ and
~<Alt>+<Shift>+<Down>~, change nesting level of a header hierarchy
with ~<Alt>+<Shift>+<Left>~ and ~<Alt>+<Shift>+<Right>~ and so on.
Have a look at the site for details
...
fields and automatically resizes table fields if longer text is
...
[[https://gist.github.com/m00natic/15410be25f7475984a71][example]] how to setup special header page with some logo in file
vayant-logo.jpg and integrate it within an Org document.  I have added
some optional information like author, title and export type as well.
