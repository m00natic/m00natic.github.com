...
mechanism but a brain-twister for novices.  Try out [[http://www.emacswiki.org/emacs/UndoTree][Undo Tree]][fn:1],
...
[[http://emacswiki.org/emacs/DiredMode][Dired]][fn:2] is the most capable file manager I've encountered.  To
start it:
...
[[http://emacswiki.org/emacs/TrampMode][TRAMP]][fn:2] is a killer.  It supports plethora of protocols.  For more
...
mail/news kitchen sink [[http://www.gnus.org/][Gnus]][fn:2].  Here's its glory documented (it
...
Database]] (BBDB)[fn:1] for managing contacts, automatic checking for
new messages for selected (mail)boxes and notifications.  For
notifications I'm using notify.el[fn:1].
...
the crippled windows eco-system, install cygwin-mount[fn:1] and then:
...
instead the default `list-buffers'[fn:2].
...
Well, get [[http://emacswiki.org/emacs/FindFileInProject][find-file-in-project]][fn:1] and then bind the same-named
...
If you happen to use [[http://git-scm.com][Git]], check out [[http://magit.github.com/magit/][Magit]][fn:1].  It's a nice
...
but none are particularly good at C++.  I use [[http://cx4a.org/software/auto-complete/][auto-complete]][fn:1]
which for completion of anything.  Here is some setup:
...
package called [[http://emacswiki.org/emacs/EmacsCodeBrowser][Emacs Code Browser]] (ECB)[fn:1] and may give you a
...
Emacs comes with a separate binary [[http://www.emacswiki.org/emacs/EmacsBrowse][ebrowse]][fn:2] that can annotate
multiple C/C++ source files and create some form of DB which can be
used for getting different kind of information for the indexed
sources.  Here's a little wrapper shell script that should be placed
somewhere on your PATH (~/usr/local/bin/~ preferably), named ebrowse++
and made executable:
...
To invoke the info reader[fn:2]:
...
[[http://emacswiki.org/emacs/Evil][Evil]][fn:1] and seems praised as the most potent emulation not just in
La la land.
...
[fn:1] Available through ~M-x list-packages~

[fn:2] This is a built-in extension
