...
|----------------------------------------------+----------------------------------------------------|
| *Emacs land (I'll call it La la land below)* | *Rest of the world*                                |
|----------------------------------------------+----------------------------------------------------|
| frame                                        | program window                                     |
| window                                       | sub window (may take the whole program window too) |
...
<tetris> and press <Enter> ...by the way this is a real command, try
it!)
...
~C-x v d PATH-TO-SOME-VERSION-CONTROLLED-FOLDER~
...
[[http://emacswiki.org/emacs/ProgrammingModes][language aware modes]] for (shell scripts / makefiles / python / java /
c / [[http://en.wikipedia.org/wiki/Scheme_(programming_language)][scheme]] / [[http://clojure.org][clojure]] / [[http://www.haskell.org][haskell]] / [[http://wiki.portal.chalmers.se/agda/pmwiki.php][agda]] / insert your favourite niche
...
for [[http://www.gnus.org/][reading or composing mail]], [[http://orgmode.org][creating presentations/books]], [[http://emacswiki.org/emacs/CategoryVersionControl][version
control]], [[http://emacswiki.org/emacs/CategoryShell][shells]], [[http://emacswiki.org/emacs/CategorySql][DB prompts]], [[http://emacswiki.org/emacs/CategoryDocumentation][composing/reading documentation]],
[[http://www.emacswiki.org/emacs/CategoryCalendar][calendar]], [[http://www.emacswiki.org/emacs/CategoryCalculators][stack calculator]], dictionaries, agendas and organizers,
spreadsheets, [[http://emacs-w3m.namazu.org/][browsing the internetz]], [[http://emacswiki.org/emacs/Rudel][collaborative editing]], [[http://www.emacswiki.org/EmacSpeak][audio
desktop]] for visually impaired, [[http://emacswiki.org/emacs/ArtistMode][ASCII drawing]], [[http://www.gnu.org/software/emms/][media collection
managers]] and [[http://www.emacswiki.org/emacs/MusicPlayers][players]], [[http://www.emacswiki.org/emacs/CategoryGames][games]], [[http://emacswiki.org/emacs/EmacsDoctor][psychotherapist]], [[http://youtu.be/D1sXuHnf_lo][controlling
vibrators]]... to name a few.  Oops, overdid it somewhere.
...
hell creature in graphics mode or from terminal or...both...several
times.  Here's how:
...
in a middle of [[http://emacswiki.org/emacs/IncrementalSearch][incremental search]] or whatever.  Suddenly you come to
...
such existential question be able to just press <y> or <n> instead:
...
Then, they were actually not that bad.  Lets redo... You are in for a
...
undos, which is roughly equivalent to redo.  This is a very powerful
mechanism but a brain-twister for novices.  Try out [[http://www.emacswiki.org/emacs/UndoTree][Undo Tree]][fn:elpa],
it visualizes the states between series of undos and changes and can
even allow you travel in time for your edits.
...
The last one is an Easter Egg.  Think slowly how useful it can be,
undoing the changes just in some selection.  [[http://www.gnu.org/software/emacs/manual/html_node/emacs/Using-Region.html][Here]] (and in the manual)
is an overview of what more is supported.
...
~C-x C-f PATH-TO-SOME-DIRECTORY~
...
~C-x C-f /ssh:akotlarski@gn1:/ezsroot/current RET~
...
~C-x C-f /sudo:root@localhost:/etc/fstab RET~
...
[[Remote%20network%20and%20sudo%20access][above]], you'll be asked for the ~.authinfo.gpg~ password once and from
...
#+BEGIN_SRC sh
machine vayant port 993 login akotlarski password very-very-secret
...
by default. The first one integrates nicely with [[http://en.wikipedia.org/wiki/Elisp][Elisp]]:
...
the crippled windows eco-system, install cygwin-mount[fn:elpa] and then:
...
Like other [[http://www.gnu.org/][GNU tools]], grep has nice integration within Emacs.  I most
...
<S> to save them all.  Job done.  As all within La la land, IBuffer is
[[http://www.emacswiki.org/emacs/IbufferMode][full of tricks and neat features]], I even have it bound to:
...
version of GNU Emacs, you can edit results in place pressing ~<e>~.
...
Cygwin environment with the settings given [[Microsoft%20Windows%20(tm)][above]].
...
selectively choose which ones to stage or unstage.  Makes partial file
commits trivial.
...
package called [[http://emacswiki.org/emacs/EmacsCodeBrowser][Emacs Code Browser]] (ECB)[fn:elpa] and may give you a
...
(eval-after-load "ecb"
    `(progn (defvar stack-trace-on-error nil)
	    ...
	    (let ((prog-path "~/Programs")) ; set folder where you store projects, not really important
...
named *ebrowse++* and made executable:
...
one (while being in another ~.emacs-project~ governed location):
...
resources I have ever seen (only second to [[http://en.wikipedia.org/wiki/Lisp_machine][Lisp Machines]] but they are
...
Mostly [[http://en.wikipedia.org/wiki/Richard_M_Stallman][Dick Stallman]] has done exceptional job, enjoy!
...
mode]].  It's often quoted as a prime reason for people starting to use
...
[[https://gist.github.com/m00natic/15410be25f7475984a71][example]] how to make header page with some picture logo and integrate
...
[[http://emacswiki.org/emacs/Evil][Evil]][fn:elpa] and seems praised as the most potent emulation not just in
La la land.

By the way, I thought editor wars finished when [[http://upsilon.cc/~zack/blog/posts/2008/10/from_Vim_to_Emacs_-_part_1/][Debian's Vim
maintainer switched to Emacs]].
...
This ~C-u NUMBER~ combination known as prefix/universal argument is a
way to pass additional argument to many commands and most often causes
them to be executed number of times.
...
e-mail buffer, copy something, switch a shell buffer, write some
...
the next line.  Now execute this 100 times.
...
Emacs actually is.  Is it an editor, an integrated development
environment, an operating system or well, a kitchen sink?  Time has
come to tell you the truth.  Luke, I'm your...

Emacs at it essence is an interpreter for a [[http://en.wikipedia.org/wiki/Lisp_%2528programming_language%2529][Lisp]] dialect - Emacs Lisp
(Elisp).  All the settings and code snippets from above are actually
Elisp program pieces.  I won't go into details how [[http://www.youtube.com/watch?v%3DHM1Zb3xmvMc][cool]] Lisp-like
languages are, [[http://www.paulgraham.com/diff.html][there]] [[http://www.paulgraham.com/avg.html][is]] a [[http://www.gigamonkeys.com/book/introduction-why-lisp.html][lot]] of [[http://www.gigamonkeys.com/book/macros-defining-your-own.html][material]] out [[http://www.defmacro.org/ramblings/lisp.html][there]].  I won't tell
you much about Elisp either for the built-in manual is outstanding:

~C-h i m Elisp~

However, I will tell you about one of my biggest Aha moments and some
practical consequences.

*** Text processing superpowers

I've come to appreciate that Elisp is the [[http://ergoemacs.org/emacs/elisp_text_processing_lang.html][most convenient programming
language for text processing]].  No joke, it's because Emacs has _a lot_
of primitives for dealing with text and buffers, after all, one of its
...
especially [[Keyboard%20macros][keyboard macros]] can get you beyond what most people would
...
3. (Optional) Jump to the definitions of the above functions and have
   a look at their code.  Not only will you see how adequate Elisp
...
very educational, though with time you may need it less often.
...
The excellent [[Documentation][documentation]] is not enough?!

** Andrey

You can always peek at [[https://github.com/m00natic/dot-emacs/][my personal settings]] as a source of mastery and
inspiration.

** Internet

Just google for it, or go to the trash bin of Emacs resources -
[[http://www.emacswiki.org/][EmacsWiki]].  Also [[http://emacsrocks.com/][here]] are some series to blow your mind.
