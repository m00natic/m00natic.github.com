...
#+TITLE: Uniform Structured Syntax, Metaprogramming and Runtime Compilation
...
#+BEGIN_EXAMPLE
  ...
  # extract from the category 1 db
...
the given DB, compiles and then calls it.
    ...
    (let ((spec (get-spec db)))  ;pull out the specification for this db
      `(funcall (compile nil (lambda ()
                               ...
                               (do-lines (line ,db) ;bind line to db entries
                                 (when ,(or (build-where where 'line spec) t)
...
Amusingly this miniscule recursive [[https://en.wikipedia.org/wiki/Source-to-source_compiler][transpiler]] corresponds to the
recursive [[WHERE matcher][match]] procedure in the *Approach I* C++ general solution.
Only the former solves the problem partly at compile time while the
latter is confined to roar only at runtime.

Here's how the macro expanded WHERE part of the code can look like:
...
straightforward code and a few declarations, the [[http://sbcl.org/][Steel Bank Common
Lisp]] compiler in particular would be able to match a C++ compiler for
the native code generation even skipping array bound checks if one
decides so.  In fact, in the same vein the [[https://edicl.github.io/cl-ppcre/][cl-ppcre]] library goes a
step further (than usual compiled languages) to build regex scanners
for constant literals (as is the case in our expansion) at compile
time thanks to a *compiler macro*.
...
- designed for (image based) system evolution, including [[http://www.gigamonkeys.com/book/object-reorientation-generic-functions.html][CLOS]][fn:4],
  the object system

Some CL implementations can leverage type declarations to rival static
language [[https://en.wikipedia.org/wiki/Ahead-of-time_compilation][AOT]] compilers[fn:5].  SBCL even provides hints where and why
...
Through [[https://en.wikipedia.org/wiki/Metaprogramming][metaprogramming]] (where Lisps excel at) one can generate
...
- the possible alternative of building explicit internal [[https://en.wikipedia.org/wiki/Abstract_syntax_tree][AST]] objects
  is more acceptable but still clunky and half-assed
...
the core language and is gradually extended and changed by the
programmer during development.  Be it by loading libraries,
re/defining functions, constants, re/setting global variables etc.

How would one produce user facing executable?  Dump the image[fn:6].
This opens opportunity to also embed data within the image before
dumping which would otherwise have to be loaded, parsed, pre/processed
during execution.
...
a global hash table for example before dump thus being available and
ready to use immediately on (user) image execution.  This would
improve executable's latency for queries.

** Alternative syntax

One may object that all falls into pieces only if we limit users to
[[https://en.wikipedia.org/wiki/S-expressions][S-expression]] DSLs.  What if forced to literary provide SQL syntax?
...
"regular" macro expansion time) of more funky syntax to good ole
S-expressions.  Doing this would allow us keep the same approach.

* Further reading

Looking at conference materials, lispers (at least the Common breed)
at times take advantage of such techniques[fn:7] for difficult
...
[fn:3] Clojure at least, incremental compilation for Scheme is wholly
dependent on the implementation.

[fn:4] See the [[http://www.lispworks.com/documentation/HyperSpec/Body/04_cf.htm][Redefining Classes]] machinery in the Common Lisp HyperSpec.

[fn:5] https://benchmarksgame-team.pages.debian.net/benchmarksgame/which-programs-are-fast.html

[fn:6] Among other things the [[https://gitlab.common-lisp.net/asdf/asdf/tree/master/uiop][uiop]] library provides portable wrapper
to dump images for various implementations.

[fn:7] Like [[https://european-lisp-symposium.org/static/2018/heisig.pdf][Petalisp - A Common Lisp Library for Data Parallel Programming]]
