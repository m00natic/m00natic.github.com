...
additional means to pull it off with great performance.

(the ideas outlined here are more thoroughly fleshed out and extended
in the [[https://github.com/m00natic/cl-fdbq][cl-fdbq]] project)
...
expands to procedure that maps over the given db, _compiles_ and then
...
reader]], courageously went onto implementing a [[https://en.wikipedia.org/wiki/One-pass_compiler][one-pass compiler]] (which
in this case is easier than [[https://en.wikipedia.org/wiki/Interpreter_(computing)][interpreter]]!) and even managed to sprinkle
some optimizations.  And on top of that, the language implementation
borrowed us its "real" compiler (at run-time!) to optimize the
expansion and generate machine code.
...
Bank Common Lisp]] compiler in particular would be able to do
respectable job.  But performance in this case will still be 4-5 times
slower than C/C++ for the following reasons:
...
around 1.5 - 2 times slower than C/C++.  But we can do better!

** Squeezing performance 2

To go further we can try to leave aside copying and converting raw
bytes and instead apply comparisons directly over them (what C/C++
does).  So I went on to employ the *ascii-strings* contrib of the
                                   ...
                                   (or ;convert filter string to bytes at macro expansion
                                    (and (search #.(ascii:string-to-ub "PET"
                                          ;the PET thing will turn to #(80 69 84)
                                                 ...
                                                 :end2 (+ offset 56))
                                         (ascii:ub-char= (aref buffer (+ offset 7))
                                                         #.(char-code #\C)));convert char to byte
                                    (and (ascii:ub-char= (aref buffer (+ offset 7))
                                                         #.(char-code #\F))
                                         (ascii:ub-string= #.(ascii:string-to-ub "MEAL") buffer
                                                           :start1 0 :end1 4
                                                           :start2 (+ offset 40)
                                                           :end2 (+ offset 56))))
                                   (ascii:ub-string< #.(ascii:string-to-ub "180620") buffer
                                                     :start1 0 :end1 6
                                                     :start2 (+ offset 14)
                                                     :end2 (+ offset 20))
                                   (ascii:ub-string< buffer buffer :start1 (+ offset 8)
                                                                   :end1 (+ offset 14)
                                                                   :start2 (+ offset 14)
                                                                   :end2 (+ offset 20)))
                            (progn
...
Now, that begins to look less like work of a human.  Summary of the
expansion changes:
...
- lazily fill parts of *line* before regex matching; in case a field is
  matched in multiple places (not the case for this query), we can
  mark its first byte position with special character at the start of
  line validation to signal that it has to be filled and can skip
  filling on subsequent operations over the same line

I won't go into detailed implementation, the previous snippets should
have shown that programmatically manipulating code in Lisp is
indistinguishable from juggling with data and these requirements are
not really rocket science.  You can see a more elaborate and complete
implementation in [[https://github.com/m00natic/cl-fdbq][cl-fdbq]].
...
(the parallelized C++ _versions_ are [[https://en.wikipedia.org/wiki/Mmap][memory map]] based, while the CL
_complete solutions_ still use sequential reading)
...
- absence of regex matcher which works over byte arrays rather than
  strings
- sequence comparisons are not [[https://en.wikipedia.org/wiki/SIMD][vectorized]] for now
...
language [[https://en.wikipedia.org/wiki/Ahead-of-time_compilation][AOT]] compilers[fn:10].  SBCL has certain ability to perform
...
As the examples show, Common Lisp actually has surprisingly rich [[http://www.lispworks.com/documentation/HyperSpec/Body/04_bc.htm][type
system]].  Combine this with [[https://en.wikipedia.org/wiki/Metaprogramming][metaprogramming]] (where Lisps excel at) and
one can generate maximally trimmed static code where needed.
...
- the possible alternative of building explicit internal [[https://en.wikipedia.org/wiki/Abstract_syntax_tree][AST]] objects
  is more acceptable but just as clunky and non composable
...
- huge majority of dynamic languages which are not Lisps are indeed
  ...
  is rather Lisp with default random syntax[fn:11])
...
undecipherable syntax noise that requires PhD to figure out.  And the
...
|----------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------------+-------------+--------------+---------+---------|
| Phase                | Description                                                                                                                                                | Control entry point | Common Lisp | Racket[fn:12] | Scheme  | Clojure |
|----------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------------+-------------+--------------+---------+---------|
...
image[fn:13].  This opens opportunity to also embed data within the
...
at times take advantage of such techniques[fn:14] for difficult
...
[fn:3] The interpreter approach in a usual language should also do
this before the iteration begins but the compiler will be long gone at
that point.
...
mocked up specific WHERE tree instance.  So not quite a real
solution either.  Yeah, I'm lazy (and life's too short for C++).

[fn:8] The [[https://ccl.clozure.com/][Clozure]] Common Lisp implementation is not to be confused
with the [[https://clojure.org/][Clojure]] language of the Lisp family.
...
[fn:10] See [[https://benchmarksgame-team.pages.debian.net/benchmarksgame/which-programs-are-fast.html][The Computer Language Benchmarks Game]] to get rough
...
[fn:11] Julia can almost match Common Lisp for this query engine
...
generated code.  In this case *Approach I* being subject to "normal"
compilation may well fare better.  In contrast, with Common Lisp we
are able to explicitly summon the compiler with full optimization over
arbitrary generated function.

[fn:12] The table is from Common Lisp's perspective but [[https://racket-lang.org/][Racket]] seems to
...
[fn:13] Among other things, the [[https://gitlab.common-lisp.net/asdf/asdf/tree/master/uiop][uiop]] library provides portable wrapper
to dump images for various implementations.

[fn:14] Like [[https://european-lisp-symposium.org/static/2018/heisig.pdf][Petalisp - A Common Lisp Library for Data Parallel Programming]]
